...
// GET /api/brands/[id]/models - Get all models for a specific brand
export async function GET(
  ...
  try {
    ...
    if (!isValidObjectId(id)) {
      console.error('Invalid brand ID format:', id);
    ...
    try {
      ...
      try {
        ...
        models = await Model.find({ brandId: id })
          .sort({ name: 1 })
          .lean();
      ...
      if (models.length === 0) {
        try {
          ...
          models = await modelsCollection
            .find({ brandId: new mongoose.Types.ObjectId(id) })
            .sort({ name: 1 })
            .toArray();
